...
if ( ! class_exists( 'Zozo_Breadcrumbs' ) ) {
	class Zozo_Breadcrumbs {
		...
		/**
		 * @var string The Breadcrumb markup
		 */
		private $breadcrumb_markup;		
		...
		public function __construct() {
			...
			$this->post 				= $queried_object;
			
			$this->options 				= $zozo_options;
			
			// Setup default array for changeable variables
			$defaults = array(
				 'separator' 				=> "/",
				 'show_post_type_archive'	=> 0,
				 'show_terms'  				=> 1,
				 'home_label'      			=> __( 'Home', 'mist' ),
				 'tag_archive_prefix'   	=> __( 'Tag:', 'mist' ),
				 'search_prefix'        	=> __( 'Search:', 'mist' ),
				 'error_prefix' 			=> __( '404 - Page not Found', 'mist' ),
			);
			
			// Setup a filter for changeable variables and meger it with the defaults
			$args 		= apply_filters( 'zozo_breadcrumbs_defaults', $defaults );
		 ...
		 * @return void
		 */				
		public function zozo_display_breadcrumbs() {
			...
			if ( function_exists('yoast_breadcrumb') && $options && $options['breadcrumbs-enable'] === true ) {
				ob_start();
					yoast_breadcrumb();
				$this->breadcrumb_markup = ob_get_clean();
			} else {
				$this->display_theme_breadcrumbs();
			}
			
			echo sprintf( '<div class="zozo-breadcrumbs">%s</div>', $this->breadcrumb_markup );
		 ...
		 * @return void
		 */				
		private function display_theme_breadcrumbs() {
					
			// Add the "Home" link
			$this->breadcrumb_markup .= $this->get_breadcrumb_home();
			...
			if ( is_singular() ) {
				...
				if ( isset( $this->post->post_type ) && get_post_type_archive_link( $this->post->post_type ) &&	$this->show_post_type_archive ) {
					$this->breadcrumb_markup .= $this->get_post_type_archive();
				}
				
				// If the post doesn't have parents
				if ( isset( $this->post->post_parent ) && $this->post->post_parent == 0 ) {
					$this->breadcrumb_markup .= $this->get_post_terms();
				// If there are parents; mostly for pages
				} else {
					$this->breadcrumb_markup .= $this->get_post_ancestors();
				}
				
				$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup();
			} else {
				// Custom post types archives
				if ( is_post_type_archive() ) {
					$this->breadcrumb_markup .= $this->get_post_type_archive( FALSE );

					// Search on custom post type (e.g. Woocommerce)
					if ( is_search() ) {
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'search' );
					}
				// Taxonomy Archives
				} elseif ( is_tax() || is_tag() || is_category() ) {
					// If we have a tag archive, add the tag prefix
					if ( is_tag() ) {
						$this->breadcrumb_markup .= $this->tag_archive_prefix;
					}
					$this->breadcrumb_markup .= $this->get_taxonomies();
					$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'term' );
				// Date Archives
				} elseif ( is_date() ) {
					global $wp_locale;
					// Set variables
					...
					if ( is_year() ) {
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'year' );
					// Month Archive, needs year link and month leaf
					} elseif ( is_month() ) {
						$this->breadcrumb_markup .= $this->get_single_breadcrumb_markup( $year, get_year_link( $year ) );
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'month' );
					// Day Archive, needs year and month link and day leaf
					} elseif ( is_day() ) {
						$this->breadcrumb_markup .= $this->get_single_breadcrumb_markup( $year, get_year_link( $year ) );
						$this->breadcrumb_markup .= $this->get_single_breadcrumb_markup( $month_name, get_month_link( $year, $month ) );
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'day' );						
					}			
				// Author Archives
				} elseif ( is_author() ) {
					$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'author' );
				// Search Page
				} elseif ( is_search() ) {
					$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'search' );
				// 404 Page
					...
					if ( class_exists( 'TribeEvents' ) && ( tribe_is_event() || is_events_archive() ) ) { 
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'events' );					
					// Default case
					} else {
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( '404' );
					}
				// bbPress
				} elseif ( class_exists( 'bbPress' ) ) {
				 	// Search Page
				 	if ( bbp_is_search() ) {
				 		$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'bbpress_search' );
				 	// User page
				 	} elseif ( bbp_is_single_user() ) {
						$this->breadcrumb_markup .= $this->get_breadcrumb_leaf_markup( 'bbpress_user' );
					}
				} 
			}
		}
		...
		private function get_breadcrumb_home() {
			...
			// If the home page is the main blog page
				...
				$blog_title = '';
				$blog_title = $this->options['zozo_blog_title'];
				
				if( isset( $blog_title ) && $blog_title == '' ) {
					$blog_title = __( 'Home', 'mist' );
				}
				$home_link = $this->get_single_breadcrumb_markup( $blog_title );
			}
		...
		private function get_post_terms() {
			...
			if ( $this->post->post_type == 'post' ) {
				$taxonomy = 'category';
			// Portfolio
			} elseif ( $this->post->post_type == 'zozo_portfolio' ) {
				$taxonomy = 'portfolio_categories';
			// Testimonials
			} elseif ( $this->post->post_type == 'zozo_testimonial' ) {
		...
		private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
			...
			if ( $separator ) {
				$separator_markup = sprintf( '<span class="zozo-breadcrumb-sep">%s</span>', $this->separator );
			}
