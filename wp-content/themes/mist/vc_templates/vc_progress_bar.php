...
if ( '' !== $bar_style ) {
	$barclass = ' bar-style-' . $bar_style;
}
$extra_bar_style = '';
if( '' !== $bar_height ) {
	$extra_bar_style = ' style="height: '.$bar_height.';"';
}
...
foreach ( $graph_lines_data as $line ) {
	if( isset( $bar_style ) && $bar_style == "default" ) {
		$unit = ( '' !== $units ) ? ' <span class="vc_label_units">' . $line['value'] . $units . '</span>' : '';
	} elseif( isset( $bar_style ) && $bar_style == "tooltip" ) {
		$unit = ( '' !== $units ) ? ' <div role="tooltip" class="tooltip top vc-label-units"><div class="tooltip-inner">' . $line['value'] . $units . '</div><div class="tooltip-arrow"></div></div>' : '';
	}
	$output .= '<div class="vc_general vc_single_bar' . ( ( isset( $line['color'] ) && 'custom' !== $line['color'] ) ? ' vc_progress-bar-color-' . $line['color'] : '' ) . '' . $barclass . '"'. $extra_bar_style .'>';
	$output .= '<h5 class="vc_label"' . $line['txtcolor'] . '>' . $line['label'] . '</h5>';
	if ( $max_value > 100.00 ) {
		$percentage_value = (float) $line['value'] > 0 && $max_value > 100.00 ? round( (float) $line['value'] / $max_value * 100, 4 ) : 0;
	} else {
		$percentage_value = $line['value'];
	}
	$output .= '<div class="vc_bar ' . esc_attr( implode( ' ', $bar_options ) ) . '" data-percentage-value="' . esc_attr( $percentage_value ) . '" data-value="' . esc_attr( $line['value'] ) . '"' . $line['bgcolor'] . '>' . $unit . '</div>';
	$output .= '</div>';
}
