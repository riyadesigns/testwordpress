	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			if ( true !== $this->is_tgmpa_complete() ) {
				...
				if ( $this->has_notices ) {				
					// Theme Edit
					if( isset( $_GET['page'] ) && $_GET['page'] == 'zozothemes-plugins' ) {
						return;
					}
		...
		public function notices() {
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				if ( $this->ispluginactive( $slug ) && false === $this->does_plugin_have_update( $slug ) ) {
					continue;
				}
				...
				if ( ! $this->is_plugin_installed( $slug ) ) {
					...
					if ( ! $this->ispluginactive( $slug ) && $this->can_plugin_activate( $slug ) ) {
				...
				if ( $install_count > 0 ) {
					$action_links['install'] = sprintf(
						...
						translate_nooped_plural( $this->strings['install_link'], $install_count, 'tgmpa' ),
							...
							esc_url( add_query_arg(
								array(
									'plugin_status' => urlencode( 'install' ),
								),
								add_query_arg(
									array(
										'page' => 'zozothemes-plugins',
			...
			if ( current_user_can( 'activate_plugins' ) && $activate_count > 0 ) {
				$action_links['activate'] = sprintf(
					...
					translate_nooped_plural( $this->strings['activate_link'], $activate_count, 'tgmpa' ),
						...
						esc_url( add_query_arg(
							array(
								'plugin_status' => urlencode( 'activate' ),
							),
							add_query_arg(
								array(
									'page' => 'zozothemes-plugins',
		...
		public function get_tgmpa_url() {
			...
			// Theme Edit
			if( isset( $_GET['return_url'] ) && $_GET['return_url'] == 'zozothemes_plugins' ) {
				$parent = 'admin.php';
		...
		public function ispluginactive( $slug ) {
			...
			return ( ( ! empty( $this->plugins[ $slug ]['is_callable'] ) && is_callable( $this->plugins[ $slug ]['is_callable'] ) ) || $ispluginactive( $this->plugins[ $slug ]['file_path'] ) );
		}
		...
		public function can_plugin_activate( $slug ) {
			return ( ! $this->ispluginactive( $slug ) && ! $this->does_plugin_require_update( $slug ) );
		}
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function categorize_plugins_to_views() {
			...
			foreach ( $this->tgmpa->plugins as $slug => $plugin ) {
				if ( $this->tgmpa->ispluginactive( $slug ) && false === $this->tgmpa->does_plugin_have_update( $slug ) ) {
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					public function bulk_install( $plugins, $args = array() ) {
						...
						/* [TGMPA - ] remove filter('upgrader_clear_destination', array($this, 'delete_old_plugin')); */
